...
const EXAMS_URL = "https://www.nibmworldwide.com/exams";
...
export const fetchExams = async (params: FetchExamsParams = {}): Promise<Exam[]> => {
  const { limit = 999, examName } = params;
  ...
  if (cached && Date.now() - cached.timestamp < CACHE_DURATION) {
    const filteredExams = examName
      ...
      : cached.data;
    return filteredExams.slice(0, limit);
  }
