...
export interface FetchLecturesResult {
  data: Lecture[];
  cacheHit: boolean;
  durationMs: number;
}

export const fetchLectures = async (params: FetchLecturesParams): Promise<FetchLecturesResult> => {
  const { date, limit: day_limit, batch, branch: branchFilter } = params;
